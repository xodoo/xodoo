...
class char_web(http.Controller):

    @http.route('/widget_char/main', type='http', auth="public", csrf=False)
    def widget_char_main(self, **kw):
        """
        地图
        :param post:
        :return:
        ...
        template = env.get_template('widget_char/main.html')
        html = template.render(object=values)
        return html

    @http.route(['/widget_char/main/data'], type='json', auth="public", cors="*", csrf=False)
    def widget_char_main_data(self, **kw):
        """
        地图
        :param post:
        :return:
