...
BASE_DIR = os.path.dirname(os.path.dirname(__file__)).replace("constroller", '')
...
class many2one_web(http.Controller):

    @http.route('/widget_many2one/main', type='http', auth="public", csrf=False)
    def widget_many2one_main(self, **kw):
        """
        地图
        :param post:
        :return:
        ...
        template = env.get_template('many2one/main.html')
        html = template.render(object=values)
        return html

    @http.route(['/widget_many2one/main/data'], type='json', auth="public", cors="*", csrf=False)
    def widget_many2one_main_data(self, **kw):
        """
        地图
        :param post:
        :return:
    ...
    @http.route(['/xodoo/widget_many2one'], type='http', auth="public", cors="*", csrf=False)
    def xodoo_widget_many2one_main_data(self, **kw):
        """
        全局
        :param post:
        :return:
        ...
        values['o'] = params
        # print(kw)
        param = pool[kw.get('resModel')].browse(int(kw.get('resId')))
        values = {
            kw.get('res_field'): int(kw.get('id')),
        }
        param.sudo().write(values)
